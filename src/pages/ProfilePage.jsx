...
const CountdownTimer = ({ deadline }) => {
  ...
  if (timeLeft === 'CLOSED') {
    return <span className="text-red-500 font-bold text-[10px] uppercase tracking-widest flex items-center gap-1"><RiTimeLine /> Registration Closed</span>;
  }

  return (
    <div className="flex flex-col items-center bg-orange-50 px-4 py-2 rounded-xl border border-orange-100">
      <span className="text-[8px] font-black uppercase tracking-widest text-gray-500 mb-0.5">Closes In</span>
      <span className="text-xs font-black text-[#cba358] tracking-wider font-mono">{timeLeft}</span>
    </div>
  );
};
...
const SelectField = ({ label, value, onChange, options, readOnly, info }) => (
  <div className="space-y-2">
    <div className="flex items-center justify-between ml-1">
      ...
      {info && (
        <span className="text-[9px] font-bold text-[#cba358] bg-orange-50 px-2 py-0.5 rounded-full">
          {info}
        </span>
      )}
    </div>
    <select 
      ...
      className={`w-full px-5 py-4 rounded-2xl border transition-all duration-300 font-bold text-slate-700 appearance-none bg-no-repeat bg-position-[right_1.25rem_center] bg-size-[1em_1em] ${
        readOnly 
          ? 'bg-slate-50/50 border-slate-100 text-gray-600 cursor-not-allowed' 
          : 'bg-white border-orange-100 focus:ring-4 focus:ring-[#cba358]/5 focus:border-[#cba358] outline-none cursor-pointer'
      }`}
  ...
  <div className="space-y-2">
    ...
    <input 
      ...
      className={`w-full px-5 py-4 rounded-2xl border transition-all duration-300 font-bold text-slate-700 ${
        readOnly 
          ? 'bg-slate-50/50 border-slate-100 text-gray-600 cursor-not-allowed' 
          : 'bg-white border-orange-100 ring-4 ring-[#cba358]/0 focus:ring-[#cba358]/5 focus:border-[#cba358] outline-none'
      }`}
    />
  </div>
);
...
const AchievementAddForm = ({ formData, setFormData }) => {
  ...
  const handleAdd = () => {
    ...
    const newAch = { title: finalTitle, date: achDate, description: achDesc, stamp: 'EFH' };
    setFormData(prev => ({
      ...prev,
  ...
  return (
    <div className="bg-gradient-to-br from-orange-50 to-indigo-50/50 border border-orange-100 rounded-[2rem] p-8 space-y-6">
      <div className="flex items-center gap-3 mb-2">
        <div className="p-2.5 bg-[#cba358] text-white rounded-xl shadow-lg shadow-[#cba358]/20">
          <RiAddLine size={18} />
        </div>
        <div>
          <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Add Your Achievement</h4>
          <p className="text-[10px] text-gray-600 font-bold mt-0.5">Will be saved when you click Save Changes</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 ml-1">Achievement Title</label>
          <select
            value={selectedTitle}
            onChange={(e) => { setSelectedTitle(e.target.value); setCustomTitle(''); }}
            className="w-full px-5 py-4 rounded-2xl border border-orange-100 bg-white text-slate-700 font-bold outline-none focus:ring-4 focus:ring-[#cba358]/5 focus:border-[#cba358] transition-all appearance-none"
        ...
        {selectedTitle === 'Other' && (
          <div className="space-y-2 md:col-span-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 ml-1">Custom Title</label>
            <input
              type="text"
              value={customTitle}
              onChange={(e) => setCustomTitle(e.target.value)}
              placeholder="e.g. Regional Karate Championship Winner 2024"
              className="w-full px-5 py-4 rounded-2xl border border-orange-100 bg-white text-slate-700 font-bold outline-none focus:ring-4 focus:ring-[#cba358]/5 focus:border-[#cba358] transition-all"
            />
          </div>
        )}
        ...
        {/* Date */}
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 ml-1">Date / Year</label>
          <input
            type="text"
            value={achDate}
        ...
        {/* Description */}
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 ml-1">Description <span className="normal-case text-gray-700">(optional)</span></label>
          <input
            type="text"
            value={achDesc}
      ...
      <button
        ...
        className={`flex items-center justify-center gap-2 w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all duration-300 ${
          added
            ? 'bg-emerald-500 text-gray-900 shadow-lg shadow-emerald-200'
            : 'bg-[#cba358] text-white hover:bg-yellow-600 shadow-lg shadow-orange-200 hover:shadow-orange-300'
        }`}
      >
...
const ProfilePage = () => {
  ...
  const allTabs = [
    { id: 'personal', label: 'Personal', icon: <RiUserLine />, roles: ['admin', 'player', 'athlete', 'coach', 'viewer', 'user'] },
    { id: 'club', label: 'Club Info', icon: <RiShieldUserLine />, roles: ['club', 'admin'] },
    { id: 'contact', label: 'Contact', icon: <RiMapPinLine />, roles: ['admin', 'player', 'athlete', 'coach', 'club', 'viewer', 'user'] },
    { id: 'documents', label: 'Documents', icon: <RiFileTextLine />, roles: ['admin', 'player', 'athlete', 'coach', 'club', 'viewer', 'user'] },
    { id: 'events', label: 'Events Hub', icon: <RiCalendarCheckLine />, roles: ['admin', 'player', 'athlete', 'coach', 'club', 'viewer', 'user'] },
    { id: 'achievements', label: 'Achievements', icon: <RiTrophyLine />, roles: ['admin', 'player', 'athlete', 'coach', 'club', 'viewer', 'user'] },
    { id: 'security', label: 'Security', icon: <RiLockPasswordLine />, roles: ['admin', 'player', 'athlete', 'coach', 'club', 'viewer', 'user'] },
  ];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] px-6">
        <div className="text-center p-10 bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 max-w-md w-full">
          <div className="w-20 h-20 bg-orange-50 text-[#cba358] rounded-full flex items-center justify-center mx-auto mb-6">
            <RiInformationLine size={40} />
          </div>
          <h2 className="text-3xl font-black text-slate-900 mb-3 tracking-tight">Not Logged In</h2>
          <p className="text-gray-500 mb-8 font-medium leading-relaxed">Please login or register to access and manage your member profile details.</p>
          <a href="/register" className="block w-full bg-[#cba358] text-white py-4 rounded-2xl font-black uppercase text-xs tracking-[0.2em] hover:bg-yellow-600 transition-all shadow-lg shadow-orange-200">
  ...
  const handleDocumentChange = async (field, file) => {
    ...
    if (!file.type.startsWith('image/')) {
      setUploadErrors(prev => ({ ...prev, [field]: '❌ Invalid file type. Please upload a valid image file.' }));
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pt-32 pb-16 lg:pt-40 lg:pb-24 font-sans">
      <div className="max-w-6xl mx-auto px-6 sm:px-10">
        
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-8">
          <div className="space-y-4 w-full">
            <div className="flex flex-wrap items-center gap-3">
              <span className="bg-white text-gray-900 text-[10px] font-black px-4 py-1.5 rounded-xl uppercase tracking-widest flex items-center gap-2">
                <RiShieldUserLine size={14} className="text-[#cba358]" />
                {user.role} Member
              </span>
              
              {/* Verification Badge */}
              {user.isVerified ? (
                <span className="bg-emerald-500 text-gray-900 text-[10px] font-black px-4 py-1.5 rounded-xl uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-emerald-500/20">
              ...
              {success && (
                <span className="flex items-center gap-1.5 text-[#cba358] text-[10px] font-black uppercase tracking-widest animate-pulse ml-2">
            ...
            <h1 className="text-5xl lg:text-7xl font-black text-slate-900 tracking-tight leading-[0.9]">
              Member <span className="text-[#cba358]">Profile.</span>
            </h1>
            <p className="text-gray-600 font-bold uppercase tracking-[0.3em] text-xs">
              Manage your EFH registration details
            </p>
          </div>
            ...
            {!isEditing ? (
              ...
              <>
                <button 
                  ...
                  className="flex-1 lg:flex-none px-8 py-5 border border-slate-200 rounded-4xl font-black text-xs uppercase tracking-[0.2em] text-gray-600 hover:bg-white transition-all"
                >
                  Cancel
                </button>
                <button 
                  ...
                  disabled={loading}
                  className="flex-1 lg:flex-none flex items-center justify-center gap-3 px-10 py-5 bg-[#cba358] text-white rounded-4xl font-black text-xs uppercase tracking-[0.2em] hover:bg-yellow-600 hover:shadow-2xl hover:shadow-[#cba358]/30 transition-all duration-500 disabled:opacity-50"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1 space-y-4">
                ...
                {tabs.map(tab => (
                  <button
                    ...
                    className={`flex items-center gap-4 px-6 py-5 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all duration-300 ${
                      activeTab === tab.id 
                        ? 'bg-[#cba358] text-white shadow-lg shadow-[#cba358]/20' 
            ...
            <div className="bg-white rounded-4xl p-12 text-gray-900 relative overflow-hidden mt-16 group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#cba358]/10 rounded-full blur-3xl -mr-16 -mt-16 group-hover:bg-[#cba358]/20 transition-all duration-700" />
              
              <div className="relative z-10 mb-6">
                {user.documents?.photograph ? (
                  <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-[#cba358]/30 shadow-2xl shadow-[#cba358]/20 group-hover:scale-105 transition-transform duration-500">
                    <img 
                  ...
                  <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-[#cba358]">
              ...
              <p className="text-gray-600 text-[10px] font-bold uppercase tracking-widest mb-6 relative z-10">Registered since {new Date(user.createdAt).toLocaleDateString()}</p>
          ...
          <div className="lg:col-span-3">
            ...
            {(user.verificationStatus === 'pending' || user.verificationStatus === 'rejected') && (
                ...
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-xl ${
                   user.verificationStatus === 'rejected' ? 'bg-red-600 text-gray-900' : 'bg-amber-500 text-gray-900'
                }`}>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 lg:p-12 shadow-2xl shadow-slate-200/40 border border-slate-100 min-h-[600px]">
                ...
                <div className="space-y-10 animate-in fade-in duration-500">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <InputField 
                      ...
                      readOnly={!isEditing || (user.role !== 'admin' && (user.isVerified || user.verificationStatus === 'pending'))}
                    ...
                    />
                    {((user.role === 'player' || user.role === 'athlete') || user.role === 'coach' || user.role === 'admin') && (
                        <SelectField 
                            label="Blood Group" 
                    ...
                    <InputField 
                        ...
                        maxLength={12}
                    />
                    {((user.role === 'player' || user.role === 'athlete') || user.role === 'admin') && (
                      <>
                        <InputField 
                          label="Father's Name" 
              ...
              {activeTab === 'contact' && (
                <div className="space-y-10 animate-in fade-in duration-500">
                  ...
                  <div className="space-y-8">
                    <h3 className="text-xs font-black uppercase tracking-widest text-gray-700">Residential Address</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               {activeTab === 'documents' && (
                <div className="space-y-10 animate-in fade-in duration-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(['player', 'athlete', 'coach', 'admin', 'user'].includes(user.role) ? [
                      ...
                      return (
                        <div key={doc.key} className={`p-6 rounded-3xl border flex flex-col gap-6 group hover:shadow-xl transition-all duration-500 ${
                          url ? 'bg-white border-slate-100' : 'bg-slate-50 border-orange-100/50'
                        }`}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                               <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${
                                 url ? 'bg-emerald-50 text-emerald-500' : 'bg-white text-[#cba358]'
                               }`}>
                            ...
                            <div className="flex items-center gap-2">
                                {url && (
                                  <button 
                                    ...
                                    className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl font-black text-[10px] uppercase tracking-widest text-[#cba358] hover:bg-[#cba358] hover:text-gray-900 transition-all shadow-sm"
                                  >
                                    View
                                  </button>
                                )}
                          ...
                          {isEditing && (user.role === "admin" || (!user.isVerified && user.verificationStatus !== "pending")) && (
                            <div className="space-y-2">
                              <div className="relative">
                                ...
                                <div className={`flex items-center justify-center gap-2 py-4 bg-white border-2 border-dashed rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
                                  uploadStatus === 'loading' ? 'border-orange-200 text-[#cba358]' :
                                  uploadErrors[doc.key] ? 'border-red-200 text-red-400' :
                                  url ? 'border-slate-100 text-gray-600 hover:border-[#cba358] hover:text-[#cba358]' :
                                  'border-orange-200 text-[#cba358] hover:bg-orange-50'
                                }`}>
                                  {uploadStatus === 'loading' ? (
                                    <>
                        ...
                        <div key={key} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col gap-6 group hover:bg-white hover:shadow-xl transition-all duration-500">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                               ...
                               <div>
                                 <p className="text-[10px] font-black uppercase tracking-widest text-gray-600 mb-1">{key.replace(/([A-Z])/g, ' $1')}</p>
                                 <div className="flex items-center gap-2">
                                   <p className="font-bold text-slate-700 text-sm">
                                     {uploadStatus === 'loading' ? 'Uploading...' : 
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {isEditing && (user.role === "admin" || (!user.isVerified && user.verificationStatus !== "pending")) && !['photograph', 'dobProof', 'idProof', 'addressProof', 'signature'].includes(key) && (
                                    <button 
                                        onClick={() => {
                                        ...
                                        className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-gray-900 transition-all"
                                    >
                          ...
                          {isEditing && (user.role === "admin" || (!user.isVerified && user.verificationStatus !== "pending")) && (
                            <div className="relative">
                              ...
                              <div className={`flex items-center justify-center gap-2 py-4 bg-white border-2 border-dashed rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
                                ...
                                'border-slate-200 text-gray-600 hover:border-[#cba358] hover:text-[#cba358]'
                              }`}>
                                {uploadStatus === 'loading' ? (
                                  <>
                                    <RiLoader4Line className="animate-spin" size={18} />
                    ...
                    {Object.keys(formData.documents || {}).length === 0 && (
                        <div className="col-span-full py-20 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-100">
                             <RiFileTextLine className="mx-auto text-gray-800 mb-4" size={48} />
                             <p className="text-gray-600 font-bold uppercase tracking-[0.2em] text-xs">No documents uploaded yet</p>
                        </div>
                    )}
                  </div>
                  ...
                  {isEditing && (
                    <div className="mt-12 pt-10 border-t border-slate-100 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="w-10 h-10 bg-orange-50 text-[#cba358] rounded-xl flex items-center justify-center">
                            <RiUploadCloud2Line size={20} />
                        </div>
                        <div>
                            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Add Additional Documents</h4>
                            <p className="text-[10px] text-gray-600 font-bold tracking-widest">UPLOAD ANY OTHER SUPPORTING FILES</p>
                        </div>
                        </div>
                        ...
                        <div className="sm:w-1/3 pt-6 sm:pt-0">
                            <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 mb-2 ml-1">Upload File</label>
                            <div className="relative">
                            <input 
                                type="file" 
                            ...
                            <div className={`px-6 py-4 rounded-2xl border-2 border-dashed transition-all flex items-center justify-center gap-2 ${
                                !customDocName.trim() ? 'bg-slate-50 border-slate-100 text-gray-700' : 
                                customDocFile ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-white border-orange-100 text-[#cba358] hover:border-orange-300'
                            }`}>
                        ...
                        <div className="sm:pt-6">
                            ...
                            className="h-[60px] px-8 bg-white text-gray-900 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-[#cba358] transition-all disabled:opacity-50"
                            >
                            Add Record
                            </button>
                        </div>
                  ...
                  {!isEditing && (
                    <div className="p-8 bg-orange-50/50 rounded-3xl border border-orange-100 mt-4">
                      <p className="text-xs font-bold text-[#cba358] leading-relaxed text-center">
              ...
              {activeTab === 'security' && (
                <div className="space-y-12 animate-in fade-in duration-500 max-w-2xl">
                  <div className="bg-orange-50 border border-orange-100 rounded-3xl p-8 mb-4">
                    <div className="flex gap-4 items-start">
                      <RiKeyLine className="text-[#cba358] shrink-0" size={24} />
                      <div>
                        <h4 className="text-gray-900 font-black text-sm uppercase tracking-wider mb-1">Update Your Security</h4>
                        <p className="text-[11px] text-yellow-600 font-bold leading-relaxed">Enter your current password to update immediately. If you've forgotten it, you can request an OTP below.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div className="relative">
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 mb-3 ml-1">Current Password</label>
                      <div className="relative">
                         <input 
                           type={showOldPass ? "text" : "password"} 
                           className="w-full px-6 py-5 bg-white border border-slate-200 rounded-2xl font-bold shadow-sm outline-none focus:border-[#cba358] transition-all pr-16"
                           value={passwordData.oldPassword}
                           onChange={(e) => handlePasswordChange('oldPassword', e.target.value)}
                         />
                         <button onClick={() => setShowOldPass(!showOldPass)} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="relative">
                        <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 mb-3 ml-1">New Password</label>
                        <div className="relative">
                          <input 
                            type={showNewPass ? "text" : "password"} 
                            className="w-full px-6 py-5 bg-white border border-slate-200 rounded-2xl font-bold shadow-sm outline-none focus:border-[#cba358] transition-all pr-12"
                          ...
                          />
                          <button onClick={() => setShowNewPass(!showNewPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-700">
                        ...
                        {(passwordData.newPassword.length > 0 || passwordError) && (
                          <div className="mt-3 p-4 bg-slate-50 border border-slate-100 rounded-2xl animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="space-y-2">
                              {[
                                { label: "At least 8 characters", valid: passwordData.newPassword.length >= 8 },
                                { label: "At least 1 uppercase letter", valid: /[A-Z]/.test(passwordData.newPassword) },
                                { label: "At least 1 number", valid: /\d/.test(passwordData.newPassword) },
                                { label: "At least 1 special character", valid: /[!@#$%^&*()_+={}\[\]:;<>,.?/~\\-]/.test(passwordData.newPassword) }
                              ].map((rule, idx) => (
                      ...
                      <div className="relative">
                        <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 mb-3 ml-1">Confirm New Password</label>
                        <div className="relative">
                          <input 
                          ...
                          />
                          <button onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-700">
                    ...
                    <div className="pt-6 space-y-8">
                       <button 
                        ...
                        className="w-full py-6 bg-white text-gray-900 rounded-3xl font-black uppercase tracking-[0.3em] text-xs shadow-xl shadow-slate-500/20 hover:bg-black transition-all active:scale-[0.98] disabled:opacity-50"
                      >
                      ...
                      <div className="relative flex items-center justify-center py-4">
                        ...
                        <span className="relative bg-white px-6 text-[9px] font-black text-gray-700 uppercase tracking-[0.3em]">OR USE OTP VERIFICATION</span>
                      </div>

                      {otpSent ? (
                        <div className="space-y-6 animate-in slide-in-from-top-4 duration-500">
                          <div className="relative">
                            <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-[#cba358] mb-3 ml-1">Verification OTP (Logged in Console)</label>
                            <input 
                              type="text" 
                              placeholder="Enter 6-digit OTP"
                              className="w-full px-6 py-6 bg-orange-50/30 border-2 border-orange-100 rounded-3xl font-black text-2xl tracking-[0.5em] text-center outline-none focus:border-[#cba358] transition-all"
                          ...
                          <button 
                            ...
                            className="w-full py-6 bg-[#cba358] text-white rounded-3xl font-black uppercase tracking-[0.3em] text-xs shadow-xl shadow-[#cba358]/20 hover:bg-yellow-600 transition-all active:scale-[0.98] disabled:opacity-50"
                          >
                        ...
                        <button 
                          ...
                          className="w-full py-5 bg-slate-50 border border-slate-100 text-gray-600 rounded-3xl font-black uppercase tracking-[0.2em] text-[10px] hover:bg-white hover:text-[#cba358] hover:border-orange-100 transition-all active:scale-[0.98]"
                        >
                ...
                <div className="space-y-10 animate-in fade-in duration-500">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                    <div>
                      ...
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600">Manage your active registrations</p>
                    </div>
                    <select 
                      value={registrationFilter}
                      onChange={(e) => setRegistrationFilter(e.target.value)}
                      className="bg-white border border-gray-200 rounded-xl px-4 py-2 outline-none font-bold text-[11px] text-gray-700 uppercase tracking-wider"
                    >
                  ...
                  {registrations.filter(reg => registrationFilter === 'All' || reg.status.toLowerCase() === registrationFilter.toLowerCase()).length === 0 ? (
                    <div className="p-16 bg-slate-50 rounded-4xl border border-dashed border-slate-200 text-center">
                      <RiCalendarCheckLine className="mx-auto text-gray-800 mb-6" size={60} />
                      <h4 className="text-xl font-bold text-gray-600 mb-2">No Active Registrations</h4>
                      <p className="text-gray-600 text-sm font-medium mb-8">Explore upcoming tournaments and events to join the EFH journey.</p>
                      <a href="/events" className="inline-block px-10 py-4 bg-white border border-slate-200 rounded-2xl font-black uppercase text-[10px] tracking-widest text-[#cba358] hover:bg-[#cba358] hover:text-gray-900 transition-all shadow-sm">
                        Browse Events
                      </a>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-6">
                      {registrations.filter(reg => registrationFilter === 'All' || reg.status.toLowerCase() === registrationFilter.toLowerCase()).map((reg, index) => (
                        <div 
                          key={reg._id} 
                          className="group relative bg-white border border-gray-200 shadow-sm rounded-3xl p-6 md:p-8 hover:shadow-2xl hover:shadow-slate-200/50 hover:border-[#cba358]/30 transition-all duration-300"
                        >
                          <div className="flex flex-col 2xl:flex-row justify-between 2xl:items-center gap-8">
                            <div className="flex items-center gap-6 flex-1 min-w-0">
                               ...
                               <div className="flex-1 min-w-0">
                                 <div className="flex flex-wrap items-center gap-2 mb-2">
                                   <span className="px-3 py-1 bg-orange-100 text-[#cba358] text-[9px] font-black uppercase tracking-[0.2em] rounded-lg whitespace-nowrap">
                                     {reg.role}
                                   </span>
                                   <span className="text-[9px] font-black text-gray-700 uppercase tracking-widest whitespace-nowrap">
                                     Joined on {new Date(reg.registrationDate).toLocaleDateString()}
                                   </span>
                                 </div>
                                 <h4 className="text-xl md:text-2xl font-black text-slate-900 group-hover:text-[#cba358] transition-colors cursor-pointer truncate">
                                   {reg.event?.title || 'Event Name Unavailable'}
                                 </h4>
                                 <p className="text-sm font-bold text-gray-500 mt-1 truncate">{reg.event?.location || 'Location Unavailable'}</p>
                               </div>
                            </div>
                            
                            <div className="flex flex-col 2xl:items-end gap-3 w-full 2xl:w-auto shrink-0">
                              <div className="flex items-center gap-3">
                                <div className={`px-5 py-2.5 border rounded-xl font-black text-[10px] uppercase tracking-widest text-center shadow-sm flex items-center gap-2 ${
                                  reg.status === 'confirmed' 
                                    ? 'bg-emerald-500 text-white border-emerald-600 shadow-emerald-500/30 shadow-lg' 
                                ...
                                <a 
                                  href={`/event/${reg.event?._id}`}
                                  className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-[10px] uppercase tracking-wider text-gray-500 hover:text-[#cba358] hover:border-orange-200 transition-all shadow-sm"
                                >
                              ...
                              {reg.status !== 'confirmed' && reg.status !== 'cancelled' && (
                                <button 
                                  ...
                                  className="px-5 py-2.5 bg-[#cba358] text-white rounded-xl font-bold text-[10px] uppercase tracking-wider hover:bg-yellow-600 transition-all shadow-lg shadow-orange-200"
                                >
                                  Pay Now
                                </button>
                              )}
                          ...
                          {reg.status === 'cancelled' && (
                            <div className="mt-6 pt-6 border-t border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
                              ...
                              <div className="flex flex-col sm:flex-row gap-3">
                                {reg.allowReapply ? (
                                  <button
                                    ...
                                    className="px-8 py-4 bg-[#cba358] disabled:bg-white disabled:text-gray-500 disabled:shadow-none text-gray-900 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-yellow-600 shadow-xl shadow-[#cba358]/20 transition-all disabled:opacity-50 shrink-0"
                                  >
                                    {loading ? 'Processing...' : 
                                  ...
                                  <div className="flex flex-col sm:flex-row items-center gap-3">
                                    {(reg.event?.registrationDeadline || reg.event?.date) && (
                                      <CountdownTimer deadline={reg.event.registrationDeadline || reg.event.date} />
                                    )}
                                    <a
                                      href={`/event/${reg.event?._id}`}
                                      className="px-8 py-4 bg-white text-gray-900 border border-gray-200 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black hover:text-white hover:border-black shadow-lg shadow-slate-900/10 transition-all text-center h-full flex items-center"
                                    >
                                      Start New Booking
                                    </a>
                                  </div>
              ...
              {activeTab === 'achievements' && (
                <div className="space-y-10 animate-in fade-in duration-500">
                  <div className="mb-12">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-600">Achievements and accolades recognized by EFH</p>
                  </div>
                  ...
                  {(!user.achievements || user.achievements.length === 0) ? (
                    <div className="py-20 text-center bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-100">
                      <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-sm border border-slate-50">
                        <RiTrophyLine size={48} className="text-gray-800" />
                      </div>
                      <h4 className="text-lg font-bold text-gray-600 mb-2">No Achievements Yet</h4>
                      <p className="text-gray-600 text-sm font-medium max-w-xs mx-auto">Participate in tournaments and events to earn recognition and accolades.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {user.achievements.map((achievement, index) => (
                        <div key={index} className="group relative p-8 bg-gradient-to-br from-white via-slate-50 to-orange-50/30 rounded-[3rem] border border-white shadow-[0_20px_50px_-20px_rgba(15,23,42,0.1)] hover:shadow-[0_40px_80px_-30px_rgba(59,130,246,0.2)] transition-all duration-700 hover:-translate-y-2 overflow-hidden flex flex-col min-h-[320px]">
                          <div className="absolute top-0 right-0 w-64 h-64 bg-orange-100/20 rounded-full blur-3xl -mr-32 -mt-32 group-hover:bg-orange-200/30 transition-all duration-700" />
                          <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-50/40 rounded-full blur-3xl -ml-24 -mb-24" />
                          <div className="absolute top-0 left-0 w-24 h-24 bg-gradient-to-br from-[#cba358]/10 to-transparent rounded-br-[4rem] group-hover:from-[#cba358]/20 transition-all duration-500" />
                          <div className="relative z-10 flex flex-col h-full">
                            <div className="flex items-center justify-between mb-10">
                              <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-xl shadow-amber-900/5 border border-amber-50 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                                <div className="p-3 bg-gradient-to-br from-amber-400 to-amber-600 text-gray-900 rounded-xl shadow-lg shadow-amber-500/20">
                            ...
                            <div className="space-y-4 flex-1">
                              <h4 className="text-2xl font-black text-slate-900 leading-tight group-hover:text-[#cba358] transition-colors line-clamp-2 uppercase tracking-tight">{achievement.title}</h4>
                              {achievement.description && (
                                <p className="text-sm font-bold text-gray-500 leading-relaxed opacity-70 line-clamp-3">{achievement.description}</p>
                              )}
                            </div>
                            <div className="mt-10 pt-8 border-t border-slate-100/50 flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <div className="w-2 h-2 rounded-full bg-[#cba358] animate-pulse shadow-[0_0_10px_rgba(37,99,235,0.5)]" />
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#cba358]">OFFICIAL RECOGNITION</span>
                              </div>
                              <div className="relative">
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 animate-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="p-10 space-y-6">
              <div className="space-y-3">
                <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gray-600 ml-1">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-2">
                <button 
                  ...
                  className="flex-1 py-4 border border-slate-200 rounded-2xl font-black text-[11px] uppercase tracking-widest text-gray-600 hover:bg-slate-50 transition-all"
                >
                  Keep Booking
                </button>
                <button 
                  ...
                  className="flex-1 py-4 bg-red-600 text-gray-900 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-red-700 shadow-xl shadow-red-200 transition-all disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-white/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 animate-in slide-in-from-bottom-8 duration-500 flex flex-col max-h-[90vh]">
            <div className="p-6 sm:p-10 border-b border-slate-50 shrink-0">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-orange-50 text-[#cba358] rounded-2xl sm:rounded-3xl flex items-center justify-center mb-4 sm:mb-6">
            ...
            <div className="p-6 sm:p-10 space-y-6 overflow-y-auto custom-scrollbar">
              <div className="bg-orange-50/50 p-5 sm:p-8 rounded-[2rem] border border-orange-100 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-24 h-24 bg-[#cba358]/5 rounded-full blur-2xl -mr-12 -mt-12" />
                
                <h4 className="text-[10px] font-black uppercase tracking-widest text-[#cba358] mb-4 flex items-center gap-2">
                   INSTRUCTIONS TO CHANGE PASSWORD
                </h4>
                
                <ul className="space-y-5">
                  <li className="flex gap-4">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl bg-white shadow-sm border border-orange-100 flex items-center justify-center shrink-0 text-[#cba358] font-black text-[10px] sm:text-xs">
                      1
                    </div>
                    <p className="text-xs sm:text-sm font-bold text-slate-600 leading-relaxed">
                      Take a clear photo of the <span className="text-slate-900 underline decoration-[#cba358] decoration-2 underline-offset-4">FRONT SIDE</span> of your Aadhaar Card.
                    </p>
                  </li>
                  <li className="flex gap-4">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl bg-white shadow-sm border border-orange-100 flex items-center justify-center shrink-0 text-[#cba358] font-black text-[10px] sm:text-xs">
                    ...
                    <p className="text-xs sm:text-sm font-bold text-slate-600 leading-relaxed">
                      Go to the <Link to="/contact" className="text-[#cba358] hover:text-yellow-700 underline underline-offset-4 transition-colors">Contact Form</Link> and raise a query.
                    </p>
                  </li>
                  <li className="flex gap-4">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl bg-white shadow-sm border border-orange-100 flex items-center justify-center shrink-0 text-[#cba358] font-black text-[10px] sm:text-xs">
                ...
                <div className="mt-6 p-4 bg-white/60 rounded-2xl border border-orange-50 shadow-sm">
                   <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-600 text-center leading-loose">
                    EFH Admin will verify your identity and update your password shortly.
                  </p>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full py-4 sm:py-5 bg-white text-gray-900 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 active:scale-[0.98]"
              >
                Understood
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center justify-center">
            ...
            <button 
              ...
              className="absolute -top-12 right-0 md:-right-12 p-2 text-white/70 hover:text-white bg-black/20 hover:bg-black/50 rounded-full transition-all"
            >
              <RiCloseLine size={32} />
            </button>
